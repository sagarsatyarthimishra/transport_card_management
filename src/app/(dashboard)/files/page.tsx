...
function isSDHFile(fileName: string) {
  return fileName.includes("SDH09066");
}
...
export default function FilesPage() {
  ...
  const [deleteTarget, setDeleteTarget] =
    useState<GeneratedFileItem | null>(null);
  ...
  async function deleteFile() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/transactions/files/${deleteTarget.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  function editFile(
    ...
    if (isSDHFile(file.fileName)) {
      router.push(
        `/transaction?fileId=${encodeURIComponent(
          file.id,
        )}`,
      );
      return;
    ...
    router.push(
      `/transactions?fileId=${encodeURIComponent(
        file.id,
      )}`,
    );
  }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Manage, edit, download and delete
