...
export async function POST(
  ...
  try {
     ...
     * VERY IMPORTANT:
     *
     * SDH ONLY searches SDHCard.
     *
     * MMM Card collection is never
     * touched here.
     */
    const cardIds =
      transactions.map(
    ...
    const fileName =
      `SALARY_SDH09066_20161229_${timestamp}.${format}`;
    ...
    /*
     * Save SDH transactions in EXACT
     * file order.
     *
     * sequence:
     *
      ...
      return NextResponse.json(
        {
          ...
          message:
            "SDH transactions and file saved successfully.",
    ...
    console.error(
      "Generate SDH transaction file error:",
      error,
    );
    ...
    return NextResponse.json(
      {
        success: false,
        message:
          "Unable to generate SDH transaction file.",
      },
      { status: 500 },
    );
  }
